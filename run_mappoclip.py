from MAPPOCMA import PPOCMA as MAPPO
...
def train(args):
    ...
    while mappo.n_episodes < MAX_EPISODES:
        ...
        print(mappo.n_episodes, '-th episode ends')

    # save the model
    mappo.save(dirs['models'], MAX_EPISODES + 2)
    current = datetime.utcnow().strftime("%b_%d_%H_%M_%S")
    plt.figure()
    plt.plot(eval_rewards)
    plt.xlabel("Episode")
    plt.ylabel("Average Reward")
    plt.legend(["MAPPO"])
    plt.savefig("MAPPOcma.png")
    plt.show()
    print('start from', now, ', end at', current)
    with open('cmappo.txt', 'w') as f:
        for item in eval_rewards:
            f.write(f"{item}\n")
