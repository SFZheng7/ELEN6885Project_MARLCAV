...
config_dir = 'configs/configs_ddqn.ini'
...
class MADoubleDQN:
    """
    An multi-agent learned with Double DQN
    """
    ...
    def train(self):
        ...
        for _ in range(10):
            ...
            # Double DQN: Use main network to select actions and target network to evaluate
            with th.no_grad():
                next_actions = self.q_network(next_states_var).argmax(dim=1, keepdim=True)
                next_q = self.target_network(next_states_var).gather(1, next_actions)

            # compute target q by: r + gamma * Q(s_{t+1}, a*)
